										...
										<tbody>
											<s:iterator value="dinDans">
												<tr>
													...
													<td><span class="status-metro status-disabled">
															<s:property value="status" />
													</span></td>
													<td><s:if test="#session.exituser.getIdentity()==1">
															<s:if test="status=='待发货'">
																<a
																	href="upOrderStatus?status=已发货&&did=<s:property value="did" />"
																	class="btn btn-info"> 确认发货 </a>
															</s:if>
														<%-- 	<s:else>
																<button class="btn btn-defalut disabled">待买家确认</button>
															</s:else> --%>
														</s:if> <s:else>
															<s:if test="status=='已发货'">
																<a href="upOrderStatus?status=已收货&&did=<s:property value="did" />&&uid=<s:property value="#session.exituser.getUid()" />"> <span
																	class="status-metro status-active">确认收货</span>
																</a>
															</s:if>
															<%-- <s:else>
																<span class="status-metro status-active disabled">待商家发货</span>
															</s:else> --%>
														</s:else></td>
												</tr>
											</s:iterator>
